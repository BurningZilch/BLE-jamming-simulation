...
% Load the CNN model
load('cnn_model.mat','net','predictorMeans','predictorStds','responseMeans','responseStds'); % This loads 'net'

% Prepare the predictors (RSSI values) for normal and jammed cases
predictors_ = data{:, 5:end};
predictors_jammed = data_jammed{:, 5:end};

predictorsNorm_ = (predictors_ - predictorMeans) ./ predictorStds;
predictorsNorm_jammed = (predictors_jammed - predictorMeans) ./ predictorStds;


% Reshape predictors for CNN input
numFeatures = size(predictorsNorm_, 2);
numSamples = size(predictorsNorm_, 1);

predictorsReshaped_ = reshape(predictorsNorm_', [numFeatures, 1, 1, numSamples]);
predictorsReshaped_jammed = reshape(predictorsNorm_jammed', [numFeatures, 1, 1, numSamples]);

% Predict positions using the trained CNN model
predictedPositionsNorm_ = predict(net, predictorsReshaped_); % Nx3 matrix
predictedPositionsNorm_jammed = predict(net, predictorsReshaped_jammed);

% Unnormalize the predicted positions
predictedPositions_ = predictedPositionsNorm_ .* responseStds + responseMeans;
predictedPositions_jammed = predictedPositionsNorm_jammed .* responseStds + responseMeans;
% *** Modification ends here ***

% Extract predicted X, Y, Z for normal and jammed cases
predictedX_ = predictedPositions_(:, 1);
predictedY_ = predictedPositions_(:, 2);
predictedZ_ = predictedPositions_(:, 3);

predictedX_jammed = predictedPositions_jammed(:, 1);
predictedY_jammed = predictedPositions_jammed(:, 2);
predictedZ_jammed = predictedPositions_jammed(:, 3);
% *** Modification ends here ***
...
%% Visualization
% Visualize the true positions and predicted positions before and after jamming
