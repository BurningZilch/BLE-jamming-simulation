...
properties
    ...
    TxPower = -10
    NodePosition (1,:) double = [0 0 0]
    AccessAddress 
    BLEChannels %store in nodes?
end
...
properties (Constant, Hidden)
    ...
    % Number of samples per symbol, the other 
    SamplesPerSymbol = 2 %this was 88 in example, but 1 or 2 are common 
...
methods
    ...
    function obj = helperBluetoothPHY(varargin)
        ...
        obj.AccessAddress = randi([0 1],32, 1); 
        obj.BLEChannels = cell(0,0);
    end
    
    % Set Tx power in dBm
    function set.TxPower(obj, value)
        validateattributes(value, {'numeric'}, {'scalar', ...
            'real', '>=', -20, '<=', 50}, mfilename, 'TxPower');
        obj.TxPower = value;
    end
